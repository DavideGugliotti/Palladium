...
type ActivityTracer () =
    do      
        ...
        listener.ShouldListenTo <- fun (_ : ActivitySource) -> true
        listener.Sample <- fun (_ : byref<ActivityCreationOptions<ActivityContext>>) -> ActivitySamplingResult.AllData
        ActivitySource.AddActivityListener(listener)
                
    member this.Writer = new ActivitySource("test_tracer")
    
type TestIoFixture () =
    let meter = new Meter("io_test_execution_meter")
    let counter = meter.CreateCounter<int>("io_test_number", "Tests")
    
    member this.CountTest() : Task =
        task{
            counter.Add(1)
        }

type DemoIOTests (tracer : ActivityTracer, ioFixture : TestIoFixture) =
    inherit Grain()
    
    interface IDemoIoTests with
    
        [<Fact>]
        member this.IoTest1() : Task =
            task {
                let activity = tracer.Writer.StartActivity("IO Test activity")
                activity.SetStartTime(DateTime.UtcNow) |> ignore
                activity.SetStatus(ActivityStatusCode.Ok) |> ignore
                activity.SetTag("MyTestActivity", "Tracing!") |> ignore
                ...
                activity.SetEndTime(DateTime.UtcNow) |> ignore
