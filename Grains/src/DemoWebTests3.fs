...
type DemoWebTests3(httpClient : TestWebClientFixture) =
    inherit Grain()
    
    interface IDemoWebTests3 with
    
        [<Fact>]
        member this.WebTest1() : Task =
            task {
                let! content = httpClient.GetResponseString("https://www.dublinlive.ie") |> Async.AwaitTask
