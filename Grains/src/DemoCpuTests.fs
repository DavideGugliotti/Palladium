...
type TestCpuFixture () =
    let meter = new Meter("cpu_test_execution_meter")
    let counter = meter.CreateCounter<int>("cpu_test_number", "Tests")
    let histogram = meter.CreateHistogram<int64>("cpu_test_response", "ms")
    
    member this.Count(delayMs : int) : Task =
        task{
...
type DemoCpuTests (cpuTest : TestCpuFixture) =
    inherit Grain()
    
    interface IDemoCpuTests with
        
        [<Fact>]
        member this.CpuTest1() : Task =
            task{
                cpuTest.Count(100) |> ignore
            }
