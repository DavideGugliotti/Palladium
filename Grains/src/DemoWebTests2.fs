...
type DemoWebTests2(httpClient : TestWebClientFixture) =
    inherit Grain()
    
    interface IDemoWebTests2 with
    
        [<Fact>]
        member this.WebTest1() : Task =
            task {
                let! content = httpClient.GetResponseString("https://www.google.ie") |> Async.AwaitTask
